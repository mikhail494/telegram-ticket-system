...
2. Upload the ZIP to ChatGPT and ask it to process every ticket and relevant attachment.
...
Notifications are disabled by default. Configure their target and provider through `/questnotify`; enablement requires a reachable target plus an authoritative, available provider. No scraper, manual production provider, or live AgentOn provider is included, so publication remains unavailable until an authoritative source is registered at runtime.
...
- Entity notification publication remains disabled until an authoritative provider is registered and configured. No scraper or production fixture provider exists.
