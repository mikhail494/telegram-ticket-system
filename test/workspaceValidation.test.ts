...
function sharedUpdate(chatId: number): Update {
  return {
    ...
    message: {
      ...
      chat_shared: { request_id: 1300, chat_id: chatId, title: "Candidate" },
    },
  };
}
...
test("non-forum workspace is rejected and not persisted", async () => {
  ...
  try {
    harness.setApiResponseOverride("getChat", () => ({
      ok: true,
      result: { id: -100700, type: "supergroup", title: "No Topics", is_forum: false },
    }));
...
test("missing bot rights produces a human-readable checklist", async () => {
  ...
  try {
    harness.setApiResponseOverride("getChatMember", (call) =>
      Number(call.payload.user_id) === 777
        ? { ok: true, result: { status: "member", user: { id: 777, is_bot: true, first_name: "Bot" } } }
        : undefined
    );
