    ...
    answers: [
      {
        ...
        reply_text: "Private batch reply",
      },
    ],
  });
}
...
test("OWNER and ADMIN receive direct private export controls while junior roles do not", async () => {
  for (const role of ["OWNER", "ADMIN", "SENIOR_AGENT", "AGENT"] as const) {
    ...
    if (role === "OWNER" || role === "ADMIN") {
      const keyboard = harness.findApiCalls("editMessageText")[0]?.payload.reply_markup as {
        inline_keyboard?: Array<Array<{ callback_data?: string }>>;
      };
      assert.equal(
        keyboard.inline_keyboard?.flat().some((button) => button.callback_data === "batch-ui:export"),
        true
      );
    }
  }
...
test("private export retires the dashboard before sending the ZIP and a fresh answer-waiting screen", async () => {
  ...
  assert.equal(
    harness.findApiCalls("sendDocument").some((call) => call.payload.chat_id === TEST_STAFF_CHAT_ID),
    false
  );
...
test("RBAC-active staff answer-package upload is redirected before preview creation", async () => {
  ...
  await harness.bot.handleUpdate(
    buildStaffDocumentUpdate({ fileName: "ticket-answers_any.json", staff: { id: 1, username: "owner" } })
  );

  assert.equal(harness.db.listTicketBatchAnswerItems("anything").length, 0);
  assert.equal(
    harness.findApiCalls("sendMessage").some((call) => String(call.payload.text).includes("private chat")),
    true
  );
});
