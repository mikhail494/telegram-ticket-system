...
describe("ticket batch export contract", () => {
  ...
  it("exports all and only active tickets in ticket/message order without mutating tickets", async () => {
    ...
    assert.deepEqual(
      snapshot.records.map((record) => record.ticket.id),
      [second.id, first.id]
    );
    assert.equal(
      snapshot.records.some((record) => record.ticket.id === closed.id),
      false
    );
  ...
  it("embeds every supported media attachment with deterministic metadata and verified bytes", async () => {
    ...
    try {
      ...
      const record = JSON.parse(strFromU8(entries["tickets.jsonl"]!).trim()) as {
        messages: Array<{ caption: string | null; media_type: string; attachments: Array<{ archive_path: string }> }>;
      };
      ...
      for (const [index, item] of mediaIndex.entries()) {
        assert.match(
          item.archive_path,
          new RegExp(`^attachments/ticket-${ticket.id}/message-\\d+/[A-Za-z0-9][A-Za-z0-9._ -]*$`)
        );
  ...
  it("keeps a known oversized Telegram attachment as unavailable while embedding the rest", async () => {
    ...
    try {
      ...
      assert.equal(
        Object.keys(entries).some((name) => name.includes("evidence.mp4")),
        false
      );
      assert.match(
        strFromU8(entries["tickets.md"]!),
...
describe("ticket answer package validation and preview", () => {
  ...
  it("rejects malformed, unsupported, duplicate, missing, extra, and invalid action packages", () => {
    ...
    for (const payload of [
      ...
      answerPackage([
        { ticket_id: 1, snapshot_token: "sha256:first", action: "no_action", reply_text: "not allowed" },
        valid[1],
        valid[2],
      ]),
      answerPackage([
        { ticket_id: 1, snapshot_token: "sha256:first", action: "reply_and_close", reply_text: "" },
        valid[1],
        valid[2],
      ]),
      answerPackage([
        { ticket_id: 1, snapshot_token: "sha256:first", action: "reply_keep_open", reply_text: "x".repeat(3501) },
        valid[1],
        valid[2],
      ]),
    ]) {
      assert.throws(
        () => parseAndValidateAnswerPackage(payload, "export_answers", exported),
        TicketBatchValidationError
      );
    }
  });
  ...
  it("builds an advisory preview with ready, stale, and inactive classifications", () => {
    ...
    assert.deepEqual(
      preview.entries.map((entry) => [entry.ticketId, entry.classification, entry.action]),
      [
...
describe("persistent answer package state machine", () => {
  function createPackage(harness: BotHarness, id = "answers_state") {
    ...
    harness.db.createTicketBatchExport({
      ...
      ticketCount: 1,
      items: [{ ticketId: ticket.id, snapshotToken: "sha256:ticket" }],
    });
    return {
      ticket,
      packageRecord: harness.db.createTicketBatchAnswerPackage({
        answerPackageId: id,
        exportId: `export_${id}`,
        staffChatId: -100900,
        packageHash: `sha256:${id}`,
        packageCreatedAt: "2026-07-30T00:00:00.000Z",
        items: [{ ticket_id: ticket.id, snapshot_token: "sha256:ticket", action: "no_action", reply_text: null }],
      }),
    };
  }
  ...
  it("keeps unresolved delivery states PARTIAL and terminal states COMPLETED", () => {
    ...
    harness.db.updateTicketBatchAnswerItem(packageRecord.answer_package_id, ticket.id, "UNKNOWN_DELIVERY", {
