...
describe("ticket answer package validation and preview", () => {
  ...
  it("accepts and previews silent_close separately from replies and no_action", () => {
    const parsed = parseAndValidateAnswerPackage(JSON.stringify({
      ...
      created_at: "2026-07-30T00:00:00.000Z",
      answers: [
        { ticket_id: 1, snapshot_token: "sha256:first", action: "silent_close", reply_text: null, follow_up_state: "NONE", internal_note: null, escalation_target: "NONE" },
        { ticket_id: 2, snapshot_token: "sha256:second", action: "no_action", reply_text: null, follow_up_state: "NONE", internal_note: null, escalation_target: "NONE" },
        { ticket_id: 3, snapshot_token: "sha256:third", action: "reply_keep_open", reply_text: "Still investigating.", follow_up_state: "NONE", internal_note: null, escalation_target: "NONE" }
      ]
    }), "export_answers", exported);
    ...
    const schema = JSON.stringify(getAnswerPackageJsonSchema());
    const instructions = buildAnswerPackageInstructions("export_answers");
    ...
    assert.match(schema, /silent_close/);
    assert.match(instructions, /`silent_close`: reply_text must be null; closes the ticket without sending a message to the user/i);
  });

  it("rejects silent_close reply text and follow-up state in version 2 packages", () => {
    const packageFor = (answer: Record<string, unknown>) => JSON.stringify({
      schema: "telegram_ticket_answer_package",
      version: 2,
    ...
    assert.throws(() => parseAndValidateAnswerPackage(packageFor({
      ticket_id: 1, snapshot_token: "sha256:silent", action: "silent_close", reply_text: "Do not send this.", follow_up_state: "NONE", internal_note: null, escalation_target: "NONE"
    }), "export_silent_validation", exportedSilent), TicketBatchValidationError);
    assert.throws(() => parseAndValidateAnswerPackage(packageFor({
      ticket_id: 1, snapshot_token: "sha256:silent", action: "silent_close", reply_text: null, follow_up_state: "WAITING_USER", internal_note: null, escalation_target: "NONE"
    }), "export_silent_validation", exportedSilent), TicketBatchValidationError);
  });
