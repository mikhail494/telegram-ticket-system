...
function answerPackage(
  ...
  ticketId: number,
  token: string,
  action: "reply_keep_open" | "reply_and_close" | "no_action" | "silent_close" = "reply_keep_open"
): string {
  return JSON.stringify({
    ...
    answers: [{ ticket_id: ticketId, snapshot_token: token, action, reply_text: action === "no_action" || action === "silent_close" ? null : "A valid reply" }]
  });
}

function multiAnswerPackage(
  exportId: string,
  answers: Array<{ ticketId: number; token: string; action: "reply_keep_open" | "reply_and_close" | "no_action" | "silent_close"; text?: string }>
): string {
  return JSON.stringify({
    schema: "telegram_ticket_answer_package",
    version: 1,
    ...
    answers: answers.map((answer) => ({
      ...
      reply_text: answer.action === "no_action" || answer.action === "silent_close" ? null : answer.text ?? `Reply for ${answer.ticketId}`
    }))
  });
}
...
describe("ticket batch Telegram workflow", () => {
  ...
  it("silently closes and archives a ticket without any user delivery", async () => {
    ...
    const token = getTicketSnapshotToken(harness.db.getTicketWithUser(ticket.id)!, harness.db.listMessagesChronological(ticket.id));
    harness.db.createTicketBatchExport({
    ...
    assert.equal(harness.db.listMessagesChronological(ticket.id).some((message) => message.direction === "STAFF_TO_USER"), false);
  ...
  it("keeps a silent close awaiting archive recovery without creating user delivery state", async () => {
    ...
    await harness.bot.handleUpdate(buildStaffDocumentUpdate({ fileName: "ticket-answers_export_silent_archive_retry.json" }));
    const preview = harness.findApiCalls("sendMessage").find((call) => String(call.payload.text).includes("Ticket answer package preview"));
    assert.ok(preview);
  ...
  it("recovers an already closed silent-close item without user delivery", async () => {
    ...
    harness.db.closeTicketRecord(ticket.id, { type: "STAFF", displayName: "Synthetic Staff", username: "synthetic_staff" });
    harness.clearApiCalls();
