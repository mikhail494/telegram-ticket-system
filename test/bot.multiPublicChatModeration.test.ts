...
import { createBotHarness, TEST_BOT_IDENTITY, type ApiMockSuccess, type BotHarness } from "./helpers/botHarness.js";
  ...
  return {
    ...
    message: {
      ...
      chat: {
        ...
        ...(messageThreadId === undefined ? {} : { is_forum: true as const }),
      },
  ...
  return {
    ...
    callback_query: {
      ...
      message: {
        message_id: messageId,
        date: 1,
        chat: { id: userId, type: "private", first_name: "Synthetic Staff" },
        text: "Dashboard",
      },
    },
  };
...
function privateText(userId: number, text: string, messageId = 1): Update {
  return {
    ...
    message: {
      message_id: messageId,
      date: 1,
      from: { id: userId, is_bot: false, first_name: "Synthetic Staff" },
      chat: { id: userId, type: "private", first_name: "Synthetic Staff" },
      text,
    },
  };
...
function setReachablePublicChat(harness: BotHarness, chatId: number): void {
  harness.setApiResponseOverride("getChat", (_call, success) => ({
    ...
    result: {
      ...
      username: "synthetic_public",
      is_forum: true,
      available_reactions: [],
    },
  }));
  harness.setApiResponseOverride("getChatMember", (call, success) =>
    call.payload.user_id === TEST_BOT_IDENTITY.id ? healthyBotMember() : success
  );
}
...
describe("multi-public-chat moderation", () => {
  ...
  it("sends grouped warnings into their originating forum topics", async () => {
    ...
    assert.deepEqual(
      warnings.map((call) => call.payload.message_thread_id),
      [101, 202]
    );
  });
  ...
  it("retains chat-level strikes when a user moves between topics", async () => {
    ...
    assert.equal(
      harness.db.listLanguageModerationViolations(CHAT_A, "1970-01-01T00:00:00.000Z")[1]?.message_thread_id,
      202
    );
  });
  ...
  it("OWNER and ADMIN can open the public-chat picker, but junior roles cannot", async () => {
    ...
    assert.equal(
      harness
        ...
        .filter((call) => String(call.payload.text).includes("Choose a public supergroup")).length,
      2
    );
    const denied = harness
      .findApiCalls("answerCallbackQuery")
      .find((call) => call.payload.callback_query_id === "callback-3");
    assert.equal(denied?.payload.show_alert, true);
    const picker = harness.findApiCalls("sendMessage")[0]?.payload.reply_markup as {
      keyboard?: Array<Array<{ request_chat?: { request_id?: number; chat_is_forum?: boolean } }>>;
    };
  ...
  it("retires a failed temporary keyboard-removal message before sending fresh public-chat settings", async () => {
    ...
    assert.ok(settings);
    assert.ok(
      harness.findApiCalls("deleteMessage").some((call) => call.payload.message_id === temporary.responseMessageId)
    );
  ...
  it("keeps permission checks and enablement inside the managed settings screen", async () => {
    ...
    assert.equal(harness.countApiCalls("editMessageText"), 1);
    assert.match(
      String(harness.findApiCalls("editMessageText")[0]?.payload.text),
  ...
  it("cancels a public-chat picker without leaving its keyboard or pending selection active", async () => {
    ...
    assert.equal(harness.findApiCalls("deleteMessage")[0]?.payload.message_id, pickerPromptId);
    assert.equal(
      (harness.findApiCalls("sendMessage")[0]?.payload.reply_markup as { remove_keyboard?: boolean }).remove_keyboard,
      true
    );
  ...
  it("explains public-chat settings and returns one fresh settings screen after validated input", async () => {
    ...
    assert.equal(
      harness.db.getManagedPublicChat(CHAT_A)?.warning_text,
