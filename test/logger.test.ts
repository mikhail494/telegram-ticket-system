...
test("logger redacts structured credentials, errors, URLs, and deep links", () => {
  ...
  const error = new Error(`failed ${secret}`);
  error.stack = `stack ${secret}`;
  logger.error(
    {
      token: secret,
      nested: { pairingToken: secret },
      link: `https://t.me/example?start=setup_${"a".repeat(24)}`,
      url: `https://api.telegram.org/bot${secret}/getMe`,
      err: error,
      ticketId: 7,
      updateId: 9,
      normalUrl: "https://example.test/help",
    },
    "safe text"
  );
  ...
  assert.match(output, /ticketId":7/);
  assert.match(output, /updateId":9/);
  assert.match(output, /example\.test\/help/);
