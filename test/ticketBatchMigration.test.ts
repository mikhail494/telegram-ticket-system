...
it("upgrades a v1.2.1 ticket batch schema through migration 19 without changing legacy exports", async () => {
  ...
  try {
    ...
    assert.deepEqual(migrationIds.map((row) => row.id), Array.from({ length: 19 }, (_, index) => index + 1));
