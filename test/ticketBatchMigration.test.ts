...
it("upgrades a v1.2.0 ticket batch schema through migration 13 without changing legacy exports", async () => {
  ...
  try {
    ...
    assert.deepEqual(migrationIds.map((row) => row.id), Array.from({ length: 13 }, (_, index) => index + 1));
