...
test("legacy ready installation still creates normal user tickets", async () => {
  ...
  try {
    harness.setApiResponseOverride("getChatMember", () => ({
      ok: true,
      result: { status: "left", user: { id: 502, is_bot: false, first_name: "User 502" } },
    }));
...
test("ready dashboard is concise and opens system status in the same message", async () => {
  ...
  try {
    ...
    assert.equal(
      keyboard.inline_keyboard?.flat().some((button) => button.callback_data === "setup:resume"),
      false
    );
    ...
    const statusKeyboard = status?.payload.reply_markup as {
      inline_keyboard?: Array<Array<{ callback_data?: string }>>;
    };
    assert.equal(
      statusKeyboard.inline_keyboard?.flat().some((button) => button.callback_data === "dashboard:home"),
      true
    );
...
test("dashboard moderation replaces the dashboard with moderation management", async () => {
  ...
  try {
    ...
    assert.equal(
      keyboard.inline_keyboard?.flat().some((button) => button.callback_data === "dashboard:public"),
      true
    );
    assert.equal(
      keyboard.inline_keyboard?.flat().some((button) => button.callback_data === "dashboard:home"),
      true
    );
  } finally {
    harness.cleanup();
...
test("team screen returns to the dashboard through the current message", async () => {
  ...
  try {
    ...
    assert.equal(
      keyboard.inline_keyboard?.flat().some((button) => button.callback_data === "rbac:preview"),
      false
    );
...
test("a staff test ticket retires its prompt and returns only its owner to a fresh dashboard after close", async () => {
  ...
  try {
    ...
    const acknowledgement = harness
      ...
      .find((call) => call.payload.chat_id === 1 && String(call.payload.text).startsWith("Thanks, your request"))!;
    assert.equal(
      harness.findApiCalls("deleteMessage").some((call) => call.payload.message_id === promptMessageId),
      true
    );
    assert.equal(
      harness.findApiCalls("deleteMessage").some((call) => call.payload.message_id === 11),
      false
    );
    ...
    assert.equal(
      harness
        .findApiCalls("deleteMessage")
        .some((call) => call.payload.message_id === acknowledgement.responseMessageId),
      false
    );
    assert.equal(
      harness
        .findApiCalls("sendMessage")
        .some((call) => call.payload.chat_id === 1 && call.payload.text === CLOSED_TEXT),
      true
    );
    assert.equal(
      harness
        .findApiCalls("sendMessage")
        .filter((call) => call.payload.chat_id === 1 && /Owner dashboard/.test(String(call.payload.text))).length,
      1
    );
    ...
    await harness.bot.handleUpdate(privateCallback(1, `user:close:${ticket.id}`, acknowledgement.responseMessageId));
    assert.equal(
      harness.findApiCalls("sendMessage").some((call) => /Owner dashboard/.test(String(call.payload.text))),
      false
    );
  } finally {
    harness.cleanup();
...
test("a failed staff test-ticket creation clears its prompt and restores the dashboard without deleting the test content", async () => {
  ...
  try {
    ...
    assert.equal(
      harness
        .findApiCalls("sendMessage")
        .some((call) => /Sorry, we could not create a support topic/.test(String(call.payload.text))),
      true
    );
    assert.equal(
      harness.findApiCalls("sendMessage").some((call) => /Owner dashboard/.test(String(call.payload.text))),
...
test("workspace picker requests a forum and required admin rights", async () => {
  ...
  try {
    ...
    const markup = harness.findApiCalls("sendMessage")[0]?.payload.reply_markup as {
      keyboard?: Array<Array<{ request_chat?: Record<string, unknown> }>>;
    };
...
test("stale private operator callbacks are inert while the current dashboard and nested public-chat controls remain active", async () => {
  ...
  try {
    ...
    assert.equal(
      harness.findApiCalls("answerCallbackQuery").every((call) => /no longer active/i.test(String(call.payload.text))),
      true
    );
  ...
  try {
    ...
    const update: Update = {
      ...
      message: {
        ...
        chat_shared: { request_id: 1300, chat_id: -100777, title: "Support", username: "support_team" },
      },
    };
  ...
  try {
    ...
    const update: Update = {
      ...
      message: {
        ...
        from: { id: 1, is_bot: false, first_name: "Owner" },
        chat: { id: 1, type: "private", first_name: "Owner" },
        chat_shared: { request_id: 1300, chat_id: TEST_STAFF_CHAT_ID, title: "Staff" },
      },
    };
    await harness.bot.handleUpdate(update);
  ...
  try {
    const update: Update = {
      ...
      message: {
        message_id: 30,
        date: 1,
        from: { id: 4, is_bot: false, first_name: "Agent" },
...
test("RBAC denies private staff controls without current workspace membership", async () => {
  ...
  harness.setApiResponseOverride("getChatMember", (call, response) =>
    Number(call.payload.user_id) === 2
  ...
  try {
    ...
    assert.match(
      String(harness.findApiCalls("answerCallbackQuery")[0]?.payload.text),
      /workspace membership required/i
    );
    assert.equal(harness.countApiCalls("sendMessage"), 0);
...
test("private invite link provides picker guidance without numeric IDs", async () => {
  ...
  const harness = createBotHarness({
    installationServiceFactory: (db) => {
      service = new InstallationService(db);
      service.consumeOwnerPairingToken(service.createOwnerPairingToken(), { telegramId: 1 });
      service.saveOnboardingStage(1, "STAFF_WORKSPACE");
      return service;
    },
  });
  try {
    ...
    assert.match(text, /cannot inspect/i);
    assert.doesNotMatch(text, /chat id/i);
  } finally {
    harness.cleanup();
  }
});
