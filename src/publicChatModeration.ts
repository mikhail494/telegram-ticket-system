...
export async function validatePublicModerationChat(
  ...
  const [chat, member] = await Promise.all([api.getChat(chatId), api.getChatMember(chatId, botId)]);
  ...
  const checks: PublicChatPermissionCheck[] = [
    ...
    {
      key: "bot_admin",
      label: "Bot is an administrator",
      passed: botAdmin,
      guidance: "Promote the bot to administrator.",
    },
    {
      key: "delete_messages",
      label: "Delete messages",
      passed: Boolean(admin?.can_delete_messages) || member.status === "creator",
      guidance: "Enable Delete Messages for the bot.",
    },
    {
      key: "restrict_members",
      label: "Restrict and ban members",
      passed: Boolean(admin?.can_restrict_members) || member.status === "creator",
      guidance: "Enable Restrict Members for the bot.",
    },
  ];
...
export function formatPublicChatPermissionChecklist(result: PublicChatValidationResult): string {
  const required = result.checks.map(
    (check) => `${check.passed ? "OK" : "MISSING"} - ${check.label}${check.passed ? "" : `: ${check.guidance}`}`
  );
  const reactions =
    result.reactionsAvailable === false
...
function reactionAvailability(reactions: readonly ReactionType[] | undefined): boolean | null {
  ...
  const availableEmoji = new Set<string>(
    reactions
      .filter((reaction): reaction is Extract<ReactionType, { type: "emoji" }> => reaction.type === "emoji")
      .map((reaction) => reaction.emoji)
  );
