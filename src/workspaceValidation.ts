...
export async function validateStaffWorkspace(
  api: Api,
  chatId: number,
  ownerId: number
  ...
  const checks: WorkspaceValidationCheck[] = [
    ...
    {
      key: "owner_admin",
      label: "Owner is an administrator",
      passed: ownerAdmin,
      guidance: "Promote the OWNER account to group administrator.",
    },
    {
      key: "bot_admin",
    ...
    },
    {
      key: "manage_topics",
      label: "Manage topics",
      passed: Boolean(rights?.can_manage_topics),
      guidance: "Enable Manage Topics for the bot.",
    },
    {
      key: "delete_messages",
      label: "Delete messages",
      passed: Boolean(rights?.can_delete_messages),
    ...
    { key: "send_messages", label: "Send messages", passed: botAdmin, guidance: "Allow the bot to send messages." },
    {
      key: "pin_messages",
      label: "Pin messages",
      passed: Boolean(rights?.can_pin_messages),
      guidance: "Enable Pin Messages for the bot.",
    },
  ];
  return {
...
export function formatWorkspaceChecklist(result: WorkspaceValidationResult): string {
  return result.checks
    .map(
      (check) =>
        `${check.passed ? "✅" : "❌"} ${check.label}${!check.passed && check.guidance ? ` — ${check.guidance}` : ""}`
    )
    .join("\n");
}
