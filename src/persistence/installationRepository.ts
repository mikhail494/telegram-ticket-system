...
export class InstallationRepository {
  ...
  claimEntityNotificationPublication(input: {
    ...
    const result = this.db
      .prepare(
        `INSERT OR IGNORE INTO entity_notification_publications (provider, entity_type, entity_id, event_type, observed_at, target_chat_id, state, first_seen_at, updated_at)
      VALUES (?, ?, ?, 'created', ?, ?, 'CLAIMED', ?, ?)`
      )
      .run(
        input.provider,
        input.entityType,
    ...
    const existing = this.db
      .prepare(
        "SELECT state FROM entity_notification_publications WHERE provider = ? AND entity_type = ? AND entity_id = ? AND event_type = 'created'"
      )
      .get(input.provider, input.entityType, input.entityId) as
      { state: EntityNotificationPublicationState } | undefined;
    if (existing?.state === "FAILED") {
      const retry = this.db
        .prepare(
          "UPDATE entity_notification_publications SET state = 'CLAIMED', observed_at = ?, target_chat_id = ?, updated_at = ?, last_error = NULL WHERE provider = ? AND entity_type = ? AND entity_id = ? AND event_type = 'created' AND state = 'FAILED'"
        )
  ...
  recordEntityNotificationPublished(
    ...
    this.db
      .prepare(
        "UPDATE entity_notification_publications SET state = 'PUBLISHED', telegram_message_id = ?, published_at = ?, updated_at = ?, last_error = NULL WHERE provider = ? AND entity_type = ? AND entity_id = ? AND event_type = 'created' AND state = 'CLAIMED'"
      )
  ...
  recordEntityNotificationFailure(
    ...
    this.db
      .prepare(
        "UPDATE entity_notification_publications SET state = 'FAILED', last_error = ?, updated_at = ? WHERE provider = ? AND entity_type = ? AND entity_id = ? AND event_type = 'created' AND state = 'CLAIMED'"
      )
  ...
  countEntityNotificationPublications(state?: EntityNotificationPublicationState): number {
    const row = state
      ? (this.db
          .prepare("SELECT COUNT(*) AS count FROM entity_notification_publications WHERE state = ?")
  ...
  getInstallationState(): InstallationStateRecord {
    return this.db
      .prepare(
        "SELECT setup_state, authorization_mode, active_workspace_id, updated_at FROM installation_state WHERE id = 1"
      )
  ...
  setInstallationState(
    ...
    this.db
      .prepare(
        `UPDATE installation_state SET setup_state = ?, authorization_mode = ?, active_workspace_id = ?, updated_at = ? WHERE id = 1`
      )
      .run(
        input.setup_state ?? current.setup_state,
        input.authorization_mode ?? current.authorization_mode,
        input.active_workspace_id === undefined ? current.active_workspace_id : input.active_workspace_id,
        now()
      );
  }
  ...
  upsertWorkspace(input: {
    ...
    this.db
      .prepare(
        `INSERT INTO workspaces (telegram_chat_id, title, username, active, imported_from_legacy, created_at, updated_at)
      VALUES (?, ?, ?, 1, ?, ?, ?)
  ...
  getWorkspaceByChatId(chatId: number): WorkspaceRecord | undefined {
    return this.db.prepare("SELECT * FROM workspaces WHERE telegram_chat_id = ?").get(chatId) as
  ...
  getActiveWorkspace(): WorkspaceRecord | undefined {
    return this.db
      .prepare(
        `SELECT w.* FROM workspaces w JOIN installation_state i ON i.active_workspace_id = w.id WHERE i.id = 1 AND w.active = 1`
      )
      .get() as WorkspaceRecord | undefined;
  }
  ...
  importManagedPublicChat(chatId: number, workspaceId: number): void {
    ...
    this.db
      .prepare(
        `INSERT INTO managed_public_chats (
  ...
  upsertManagedPublicChat(input: {
    ...
    this.db
      .prepare(
        `INSERT INTO managed_public_chats (chat_id, workspace_id, title, username, is_forum, active, imported_from_legacy, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, 1, 0, ?, ?)
  ...
  getManagedPublicChat(chatId: number, includeInactive = false): ManagedPublicChatRecord | undefined {
    const row = this.db
      .prepare(`SELECT * FROM managed_public_chats WHERE chat_id = ?${includeInactive ? "" : " AND active = 1"}`)
  ...
  listManagedPublicChats(includeInactive = false): ManagedPublicChatRecord[] {
    const rows = this.db
      .prepare(
        `SELECT * FROM managed_public_chats${includeInactive ? "" : " WHERE active = 1"} ORDER BY title COLLATE NOCASE, chat_id`
      )
  ...
  updateManagedPublicChatConfig(
    ...
    const result = this.db
      .prepare(
        `UPDATE managed_public_chats SET warning_text = ?, allowlist_json = ?,
      warning_cooldown_minutes = ?, warning_message_threshold = ?, lookback_minutes = ?, updated_at = ?
      WHERE chat_id = ? AND active = 1`
      )
      .run(
        input.warningText.trim(),
        JSON.stringify(normalizeManagedChatAllowlist(input.allowlist)),
        input.warningCooldownMinutes,
  ...
  setManagedPublicChatModerationEnabled(chatId: number, enabled: boolean): boolean {
    return (
      this.db
        .prepare(
          "UPDATE managed_public_chats SET moderation_enabled = ?, updated_at = ? WHERE chat_id = ? AND active = 1"
        )
  ...
  recordManagedPublicChatPermissionHealth(input: {
    ...
    return (
      this.db
        .prepare(
          `UPDATE managed_public_chats SET permission_status = ?, reaction_status = ?,
      ...
      permissions_checked_at = ?, updated_at = ? WHERE chat_id = ? AND active = 1`
        ...
        .run(
          input.healthy ? "HEALTHY" : "UNHEALTHY",
          input.reactionsAvailable === null ? "UNKNOWN" : input.reactionsAvailable ? "AVAILABLE" : "UNAVAILABLE",
          input.connected === undefined ? null : input.connected ? "CONNECTED" : "UNREACHABLE",
          input.title === undefined ? 0 : 1,
          input.title ?? null,
          input.username === undefined ? 0 : 1,
          input.username ?? null,
          input.isForum === undefined ? 0 : 1,
          input.isForum ? 1 : 0,
          now(),
          now(),
          input.chatId
  ...
  recordManagedPublicChatUnreachable(chatId: number): boolean {
    return (
      this.db
        .prepare(
          `UPDATE managed_public_chats SET connection_status = 'UNREACHABLE',
  ...
  deactivateManagedPublicChat(chatId: number): boolean {
    return (
      this.db
        .prepare(
          "UPDATE managed_public_chats SET active = 0, moderation_enabled = 0, updated_at = ? WHERE chat_id = ? AND active = 1"
        )
  ...
  getTeamMember(userId: number): TeamMemberRecord | undefined {
    return this.db.prepare("SELECT * FROM team_members WHERE user_telegram_id = ? AND active = 1").get(userId) as
  ...
  listTeamMembers(): TeamMemberRecord[] {
    return this.db
      .prepare(
        "SELECT * FROM team_members WHERE active = 1 ORDER BY CASE role WHEN 'OWNER' THEN 0 WHEN 'ADMIN' THEN 1 WHEN 'SENIOR_AGENT' THEN 2 ELSE 3 END, user_telegram_id"
      )
  ...
  upsertTeamMember(input: {
    ...
    this.db
      .prepare(
        `INSERT INTO team_members (user_telegram_id, username, display_name, role, active, added_by, created_at, updated_at)
      VALUES (?, ?, ?, ?, 1, ?, ?, ?)
  ...
  revokeTeamMember(userId: number): boolean {
    return (
      this.db
        .prepare(
          "UPDATE team_members SET active = 0, updated_at = ? WHERE user_telegram_id = ? AND active = 1 AND role != 'OWNER'"
        )
  ...
  transferOwner(newOwnerId: number): void {
    this.db.transaction(() => {
      const oldOwner = this.db
        .prepare("SELECT user_telegram_id FROM team_members WHERE role = 'OWNER' AND active = 1")
        .get() as { user_telegram_id: number } | undefined;
      if (!oldOwner) throw new Error("An active owner is required for transfer.");
      const timestamp = now();
      this.db
        .prepare("UPDATE team_members SET role = 'ADMIN', updated_at = ? WHERE user_telegram_id = ?")
        .run(timestamp, oldOwner.user_telegram_id);
      this.db
        .prepare(
          `INSERT INTO team_members (user_telegram_id, role, active, created_at, updated_at) VALUES (?, 'OWNER', 1, ?, ?)
        ON CONFLICT(user_telegram_id) DO UPDATE SET role = 'OWNER', active = 1, updated_at = excluded.updated_at`
        )
        .run(newOwnerId, timestamp, timestamp);
    })();
  }

  invalidateUnconsumedTokens(kind: SecureTokenRecord["kind"]): void {
    this.db
      .prepare("UPDATE secure_setup_tokens SET consumed_at = ? WHERE kind = ? AND consumed_at IS NULL")
      .run(now(), kind);
  }
  ...
  insertSecureToken(input: {
    ...
    this.db
      .prepare(
        `INSERT INTO secure_setup_tokens (token_hash, kind, role, created_by, expires_at, created_at) VALUES (?, ?, ?, ?, ?, ?)`
      )
  ...
  listUnconsumedTokens(kind?: SecureTokenRecord["kind"]): SecureTokenRecord[] {
    return (
      kind
        ...
        : this.db.prepare("SELECT * FROM secure_setup_tokens WHERE consumed_at IS NULL").all()
    ) as SecureTokenRecord[];
  }
  ...
  consumeOwnerTokenAndCreateOwner(
    ...
    return this.db.transaction(() => {
      const token = this.db
        .prepare("SELECT * FROM secure_setup_tokens WHERE id = ? AND consumed_at IS NULL AND expires_at > ?")
        .get(tokenId, at) as SecureTokenRecord | undefined;
      ...
      const timestamp = now();
      this.db
        .prepare("UPDATE secure_setup_tokens SET consumed_at = ?, claimed_by = ? WHERE id = ? AND consumed_at IS NULL")
        .run(timestamp, user.telegramId, tokenId);
      this.upsertUserForControl(user);
      if (existing) {
        this.db
          .prepare(
            "INSERT OR REPLACE INTO owner_transfer_confirmations (claimant_telegram_id, token_id, created_at) VALUES (?, ?, ?)"
          )
          .run(user.telegramId, tokenId, timestamp);
      ...
      this.db
        .prepare(
          "UPDATE secure_setup_tokens SET consumed_at = ? WHERE kind IN ('OWNER_PAIRING','OWNER_RECOVERY') AND consumed_at IS NULL"
        )
        .run(timestamp);
      return "PAIRED";
    })();
  ...
  invalidateTokenAndAssignMember(tokenId: number, user: UserInput, role: TeamRole, at: string): void {
    this.db.transaction(() => {
      const token = this.db
        .prepare(
          "SELECT id FROM secure_setup_tokens WHERE id = ? AND kind = 'TEAM_INVITE' AND consumed_at IS NULL AND expires_at > ?"
        )
        .get(tokenId, at);
      ...
      this.db
        .prepare("UPDATE secure_setup_tokens SET consumed_at = ?, claimed_by = ? WHERE id = ? AND consumed_at IS NULL")
        .run(now(), user.telegramId, tokenId);
    })();
  }

  hasPendingOwnerTransfer(userId: number): boolean {
    return Boolean(
      this.db.prepare("SELECT 1 FROM owner_transfer_confirmations WHERE claimant_telegram_id = ?").get(userId)
    );
  }
  ...
  saveOnboardingSession(userId: number, stage: string, state = "ACTIVE", candidateChatId?: number | null): void {
    this.db
      .prepare(
        `INSERT INTO onboarding_sessions (user_telegram_id, stage, state, candidate_chat_id, updated_at) VALUES (?, ?, ?, ?, ?)
  ...
  getOnboardingSession(userId: number): OnboardingSessionRecord | undefined {
    return this.db.prepare("SELECT * FROM onboarding_sessions WHERE user_telegram_id = ?").get(userId) as
      OnboardingSessionRecord | undefined;
  }

  setOnboardingPrimaryMessage(userId: number, chatId: number | null, messageId: number | null): void {
    this.db
      .prepare(
        "UPDATE onboarding_sessions SET primary_message_chat_id = ?, primary_message_id = ?, updated_at = ? WHERE user_telegram_id = ?"
      )
  ...
  getInstallationOperationalCounts(): {
    ...
    return {
      ...
      moderationEnabled: scalar(
        "SELECT COUNT(*) AS count FROM managed_public_chats WHERE active = 1 AND moderation_enabled = 1"
      ),
      unhealthyModerationChats: scalar(
        "SELECT COUNT(*) AS count FROM managed_public_chats WHERE active = 1 AND permission_status = 'UNHEALTHY'"
      ),
      pendingCleanup: scalar(
        "SELECT COUNT(*) AS count FROM language_moderation_cleanup_jobs WHERE state != 'COMPLETED'"
      ),
      pendingArchives: scalar("SELECT COUNT(*) AS count FROM tickets WHERE status = 'CLOSED' AND archived_at IS NULL"),
      pendingBatchStaffOperations: scalar(
        "SELECT COUNT(*) AS count FROM ticket_batch_answer_packages WHERE final_summary_state IN ('PENDING','FAILED')"
      ),
    };
  }
}
