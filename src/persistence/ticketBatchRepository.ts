...
export class TicketBatchRepository {
  ...
  getTicketBatchAnswerPackage(
    ...
    return this.db
      .prepare("SELECT * FROM ticket_batch_answer_packages WHERE answer_package_id = ? AND staff_chat_id = ?")
  ...
  getTicketBatchAnswerPackageByHash(
    ...
    return this.db
      .prepare("SELECT * FROM ticket_batch_answer_packages WHERE package_hash = ? AND staff_chat_id = ?")
  ...
  getTicketBatchAnswerPackageByPreviewToken(
    ...
    return this.db
      .prepare("SELECT * FROM ticket_batch_answer_packages WHERE preview_token = ? AND staff_chat_id = ?")
  ...
  createTicketBatchAnswerPackage(input: CreateTicketBatchAnswerPackageInput): TicketBatchAnswerPackageRecord {
    const tx = this.db.transaction((value: CreateTicketBatchAnswerPackageInput) => {
      ...
      this.db
        .prepare(
          `INSERT INTO ticket_batch_answer_packages (answer_package_id, export_id, staff_chat_id, package_hash, source_chat_id, source_message_id, package_created_at, imported_at, status, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'PENDING', ?)`
        )
        .run(
          value.answerPackageId,
          value.exportId,
          value.staffChatId,
          value.packageHash,
          value.sourceChatId ?? null,
          value.sourceMessageId ?? null,
          value.packageCreatedAt,
          timestamp,
          timestamp
        );
      const insert = this.db.prepare(
        `INSERT INTO ticket_batch_answer_items (answer_package_id, ticket_id, snapshot_token, action, reply_text, state, updated_at, follow_up_state, internal_note, escalation_target, topic_echo_state) VALUES (?, ?, ?, ?, ?, 'PENDING', ?, ?, ?, ?, 'PENDING')`
      );
      for (const item of value.items)
        insert.run(
          value.answerPackageId,
  ...
  listTicketBatchAnswerItems(answerPackageId: string): TicketBatchAnswerItemRecord[] {
    return this.db
      .prepare("SELECT * FROM ticket_batch_answer_items WHERE answer_package_id = ? ORDER BY ticket_id ASC")
  ...
  getLatestTicketBatchDeliveryFailure(
    ...
    const row = this.db
      .prepare(
        `SELECT i.delivery_error_category AS category, i.delivery_error_permanence AS permanence,
      ...
      .get(ticketId, staffChatId) as
      | (Omit<TicketBatchDeliveryFailureContext, "staff_failure_event_posted"> & { staff_failure_event_posted: number })
      | undefined;
  ...
  getLatestTicketBatchStaffSyncContext(ticketId: number, staffChatId: number): TicketBatchStaffSyncContext | undefined {
    const row = this.db
      .prepare(
        `SELECT i.topic_echo_state AS state, i.topic_echo_message_id IS NOT NULL AS delivered,
      ...
      .get(ticketId, staffChatId) as
      | (Omit<TicketBatchStaffSyncContext, "delivered" | "internal_context_available"> & {
          delivered: number;
          internal_context_available: number;
        })
      | undefined;
    return row
      ? {
  ...
  updateTicketBatchAnswerPackagePreviewPage(answerPackageId: string, staffChatId: number, page: number): void {
    this.db
      .prepare(
        "UPDATE ticket_batch_answer_packages SET preview_page = ?, updated_at = ? WHERE answer_package_id = ? AND staff_chat_id = ? AND status = 'PENDING'"
      )
  ...
  clearTicketBatchAnswerPackagePreview(answerPackageId: string, staffChatId: number): void {
    this.db
      .prepare(
        "UPDATE ticket_batch_answer_packages SET preview_token = NULL, preview_chat_id = NULL, preview_message_id = NULL, preview_page = NULL, updated_at = ? WHERE answer_package_id = ? AND staff_chat_id = ?"
      )
  ...
  cancelTicketBatchAnswerPackage(answerPackageId: string, staffChatId: number): boolean {
    const result = this.db
      .prepare(
        "UPDATE ticket_batch_answer_packages SET status = 'CANCELLED', updated_at = ? WHERE answer_package_id = ? AND staff_chat_id = ? AND status = 'PENDING'"
      )
      .run(now(), answerPackageId, staffChatId);
    return result.changes === 1;
  }
  ...
  claimTicketBatchAnswerItem(answerPackageId: string, ticketId: number): boolean {
    const result = this.db
      .prepare(
        "UPDATE ticket_batch_answer_items SET state = 'APPLYING', updated_at = ? WHERE answer_package_id = ? AND ticket_id = ? AND state = 'PENDING'"
      )
  ...
  listPendingTicketBatchFinalSummaries(staffChatId: number, at: string, limit = 20): TicketBatchAnswerPackageRecord[] {
    return this.db
      .prepare(
        `SELECT * FROM ticket_batch_answer_packages
      ...
      ORDER BY updated_at ASC, answer_package_id ASC LIMIT ?`
      )
      .all(staffChatId, at, limit) as TicketBatchAnswerPackageRecord[];
  }

  recordTicketBatchFinalSummaryAttempt(answerPackageId: string, staffChatId: number): void {
    this.db
      .prepare(
        `UPDATE ticket_batch_answer_packages SET final_summary_attempt_count = final_summary_attempt_count + 1, updated_at = ?
      WHERE answer_package_id = ? AND staff_chat_id = ?`
      )
      .run(now(), answerPackageId, staffChatId);
  }
  ...
  recordTicketBatchFinalSummarySent(answerPackageId: string, staffChatId: number, messageId: number): void {
    this.db
      ...
      .run(messageId, now(), now(), now(), answerPackageId, staffChatId);
  }
  ...
  recordTicketBatchFinalSummaryFailure(
    ...
    this.db
      ...
      .run(state, error, nextRetryAt, error, now(), now(), answerPackageId, staffChatId);
  }
  ...
  recordTicketBatchTopicEcho(
    ...
    this.db
      ...
      .run(
        ...
        options.nextRetryAt ?? null,
        options.incrementAttempt ? 1 : 0,
        options.diagnostic?.category ?? null,
        options.diagnostic?.telegramErrorCode ?? null,
        options.diagnostic?.httpStatus ?? null,
        options.diagnostic?.method ?? null,
        options.diagnostic?.description ?? null,
        state === "TERMINAL_FAILED" ? 1 : 0,
        now(),
        now(),
        answerPackageId,
        ticketId
  ...
  getTicketBatchRecoveryAudit(staffChatId: number, at: string): TicketBatchRecoveryAudit {
    ...
    const count = (where: string): number =>
      (
        this.db
          .prepare(
            `SELECT COUNT(*) AS count FROM ticket_batch_answer_items i JOIN ticket_batch_answer_packages p ON p.answer_package_id = i.answer_package_id WHERE ${base} AND ${where}`
          )
          .get({ staffChatId, at }) as { count: number }
      ).count;
    const finalSummaries = (
      this.db
        .prepare(
          `SELECT COUNT(*) AS count FROM ticket_batch_answer_packages WHERE staff_chat_id = ? AND final_summary_state IN ('PENDING', 'FAILED') AND (final_summary_next_retry_at IS NULL OR final_summary_next_retry_at <= ?)`
        )
        .get(staffChatId, at) as { count: number }
    ).count;
    return {
      successTopicEchoes: count(
        `i.topic_echo_state IN ('PENDING','FAILED') AND ${due} AND i.action IN ('reply_keep_open','reply_and_close') AND i.delivery_message_id IS NOT NULL AND i.delivery_error_category IS NULL AND i.delivery_error_permanence IS NULL AND i.delivery_failure_event_state != 'SENT'`
      ),
      failureEvents: count(
        "i.delivery_failure_event_state IN ('PENDING','FAILED') AND i.action IN ('reply_keep_open','reply_and_close') AND i.delivery_error_category IS NOT NULL AND i.delivery_message_id IS NULL AND (i.delivery_failure_event_next_retry_at IS NULL OR i.delivery_failure_event_next_retry_at <= @at)"
      ),
      noActionFollowUpEvents: count(
        `i.topic_echo_state IN ('PENDING','FAILED') AND ${due} AND i.action = 'no_action' AND (i.follow_up_state != 'NONE' OR i.internal_note IS NOT NULL OR i.escalation_target != 'NONE')`
      ),
      finalSummaries,
      invalidSuccessEchoes: count(
        `i.topic_echo_state IN ('PENDING','FAILED') AND i.action IN ('reply_keep_open','reply_and_close') AND (i.delivery_message_id IS NULL OR i.delivery_error_category IS NOT NULL OR i.delivery_error_permanence IS NOT NULL OR i.delivery_failure_event_state = 'SENT')`
      ),
  ...
  finalizeTicketBatchAnswerPackage(
    ...
    this.db
      .prepare(
        "UPDATE ticket_batch_answer_packages SET status = ?, completed_at = CASE WHEN ? THEN ? ELSE completed_at END, updated_at = ? WHERE answer_package_id = ? AND staff_chat_id = ?"
      )
      .run(
