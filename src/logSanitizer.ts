...
export function sanitizeLogString(value: string): string {
  return value
    .replace(apiUrl, "$1[REDACTED]")
    .replace(deepLink, "$1_[REDACTED]")
    .replace(bearer, (match) =>
      match.startsWith("Authorization") ? "Authorization: Bearer [REDACTED]" : "Bearer [REDACTED]"
    )
    .replace(botToken, REDACTED);
}
    ...
    return {
      ...
      ...Object.fromEntries(
        Object.entries(value).map(([key, nested]) => [
          key,
          secretField.test(key) ? REDACTED : sanitizeLogValue(nested, seen),
        ])
      ),
    };
