...
async function hiddenPrompt(): Promise<string> {
  if (!process.stdin.isTTY || !process.stdout.isTTY)
    throw new Error(
      "BOT_TOKEN is missing in non-interactive mode. Set it in the environment or run npm run setup in a terminal."
    );
...
async function confirmTokenOverwrite(): Promise<boolean> {
  ...
  const answer = await new Promise<string>((resolve) =>
    prompt.question("A different BOT_TOKEN is already saved. Replace it? Type REPLACE: ", resolve)
  );
  prompt.close();
...
async function verifyWithTelegram(token: string): Promise<{ id: number; username: string }> {
  ...
  if (!response.ok || !body.ok || !body.result?.id || !body.result.username)
    throw new Error(`Telegram rejected the bot token: ${body.description ?? response.statusText}`);
  return { id: body.result.id, username: body.result.username };
}

export async function runSetup(
  options: SetupOptions = {}
): Promise<{ botId: number; botUsername: string; envPath: string | null }> {
  ...
  if (!/^\d{5,}:[A-Za-z0-9_-]{10,}$/.test(token))
    throw new Error("BOT_TOKEN has an invalid format. Paste the complete token from BotFather.");
  ...
  if (envPath) {
    const values = {
      ...existing,
      BOT_TOKEN: tokenToPersist ?? token,
      DATABASE_URL: env.DATABASE_URL ?? existing.DATABASE_URL ?? "file:./data/support.db",
      LOG_LEVEL: env.LOG_LEVEL ?? existing.LOG_LEVEL ?? "info",
    };
    ...
    try {
      ...
      /* Windows may not implement POSIX modes. */
    }
  }
