...
export class PrivateControlPlane {
  ...
  private readonly pendingPublicChatConfigurations = new Map<
    number,
    { chatId: number; field: PublicChatConfigurationField }
  >();
  ...
  async showQuickReplyDetail(ctx: Context, templateId: string, notice?: string): Promise<void> {
    ...
    if (!template) {
      await this.renderScreen(
        ctx,
        "This Quick Reply is no longer available.",
        new InlineKeyboard().text("Back", "quick:list")
      );
      return;
    }
    await this.renderScreen(
      ctx,
      ["Quick reply", "", `Name: ${template.title}`, "", "Text:", template.text, ...(notice ? ["", notice] : [])].join(
        "\n"
      ),
      new InlineKeyboard()
  ...
  async beginQuickReplyInput(
    ctx: Context,
    pending: Exclude<PendingQuickReplyInput, { kind: "ADD_PREVIEW" }>
  ): Promise<void> {
    if (!ctx.from) return;
    this.setPendingQuickReplyInput(ctx.from.id, pending);
    await this.retireScreens(ctx);
    await this.sendFreshScreen(
      ctx,
      this.quickReplyInputPrompt(pending),
      new InlineKeyboard().text("Back", "quick:list")
    );
  }
  ...
  async showQuickReplyCategoryPicker(ctx: Context): Promise<void> {
    const keyboard = new InlineKeyboard();
    for (const category of this.operatorDependenciesOrThrow().quickReplies.listCategories())
      keyboard.text(category.title, `quick:add-category:${category.id}`).row();
  ...
  async consumeQuickReplyInput(ctx: Context, text: string): Promise<boolean> {
    ...
    const invalid = (maximum: number): string | undefined =>
      !trimmed ? "value cannot be empty." : trimmed.length > maximum ? `use at most ${maximum} characters.` : undefined;
    if (pending.kind === "EDIT_TITLE" || pending.kind === "ADD_TITLE") {
      const error = invalid(32);
      if (error) {
        await this.renderScreen(
          ctx,
          this.quickReplyInputPrompt(pending, error),
          new InlineKeyboard().text("Back", "quick:list")
        );
        return true;
      }
    ...
    if (pending.kind === "EDIT_TEXT" || pending.kind === "ADD_TEXT") {
      ...
      if (pending.kind === "EDIT_TEXT") {
        ...
        const next: PendingQuickReplyInput = {
          kind: "ADD_PREVIEW",
        ...
        await this.renderScreen(
          ctx,
          ["New Quick reply", "", `Name: ${next.title}`, "", "Text:", next.text].join("\n"),
          new InlineKeyboard().text("Save", "quick:add-save").text("Cancel", "quick:list")
        );
      }
    }
    return true;
  ...
  async beginSupportAcknowledgementInput(ctx: Context): Promise<void> {
    ...
    await this.renderScreen(
      ctx,
      this.supportAcknowledgementPrompt(),
  ...
  async consumeSupportSettingsInput(ctx: Context, text: string): Promise<boolean> {
    ...
    if (pending === "RESPONSE_TIME") {
      ...
      if (!normalized.value) {
        await this.renderScreen(
          ctx,
          this.supportResponseTimePrompt(normalized.error),
          new InlineKeyboard().text("Back", "support:back")
        );
        return true;
      }
      ...
      if (rendered.error) {
        await this.renderScreen(
          ctx,
          this.supportResponseTimePrompt(
            "that response time makes the current acknowledgement too long. Shorten it or edit the acknowledgement first."
          ),
          new InlineKeyboard().text("Back", "support:back")
        );
        return true;
    ...
    if (!normalized.value) {
      await this.renderScreen(
        ctx,
        this.supportAcknowledgementPrompt(normalized.error),
        new InlineKeyboard().text("Back", "support:back")
      );
      return true;
    }
  ...
  async handlePrivateInput(ctx: Context, text: string): Promise<boolean> {
    ...
    if (isPrivateInviteLink(text)) {
      await this.clearPublicChatPicker(ctx);
      await this.showPublicChats(
        ctx,
        "The Bot API cannot inspect an inaccessible private invite link. Add the bot to the group, then use the Telegram public-chat picker. You do not need a numeric chat ID."
      );
      return true;
    }
    ...
    try {
      ...
      await this.showPublicChats(
        ctx,
        "That public supergroup could not be validated. Check its public username and the bot permissions, or use the Telegram picker."
      );
    }
    return true;
  }
  ...
  async handleCallback(ctx: Context, data: string): Promise<boolean> {
    ...
    if (namespace === "rbac") {
      await ctx.answerCallbackQuery({
        text: "Role-based access is automatic for ready installations.",
        show_alert: true,
      });
      return true;
    }
  ...
  async showModerationDashboard(ctx: Context): Promise<void> {
    ...
    const text = [
      "Moderation",
      "",
      `Managed public chats: ${chats.length}`,
      `Enabled: ${enabled}`,
      `Needs attention: ${unhealthy}`,
      "",
      "Manage each public chat's moderation settings and permission health.",
    ].join("\n");
    await this.renderScreen(
      ctx,
      text,
      new InlineKeyboard().text("Manage public chats", "dashboard:public").row().text("Back", "dashboard:home")
    );
  }
  ...
  async showPublicChats(ctx: Context, notice?: string): Promise<void> {
    ...
    for (const chat of chats)
      keyboard.text(`Open settings: ${this.publicChatButtonLabel(chat)}`, `public:open:${chat.chat_id}`).row();
    keyboard.text("Back", "dashboard:home");
    const lines = chats.length
      ? chats.flatMap((chat) => [
          "",
          this.publicChatLabel(chat),
          chat.username ? `@${chat.username}` : "No public username",
          `Connected: ${this.publicChatConnectionLabel(chat)}`,
  ...
  async inspectAndSavePublicChat(
    ctx: Context,
    chatId: number,
    ...
    dependencies.db.upsertManagedPublicChat({
    ...
    dependencies.db.recordManagedPublicChatPermissionHealth({
      chatId: result.chatId,
    ...
    await this.clearPublicChatPicker(ctx);
    await this.showPublicChatSettings(
      ctx,
      result.chatId,
      result.valid
  ...
  getPendingPublicChatConfiguration(
    userId: number
  ): { chatId: number; field: PublicChatConfigurationField } | undefined {
    return this.pendingPublicChatConfigurations.get(userId);
  }

  setPendingPublicChatConfiguration(
    userId: number,
    input: { chatId: number; field: PublicChatConfigurationField }
  ): void {
  ...
  async renderScreen(ctx: Context, text: string, replyMarkup: InlineKeyboard): Promise<void> {
    ...
    const callbackTarget =
      callbackMessage?.chat.type === "private"
        ...
        : undefined;
    const target =
      (ctx.from ? this.activeScreens.get(ctx.from.id) : undefined) ??
      (ctx.from ? this.persistedScreen(ctx.from.id) : undefined) ??
      callbackTarget;
    const temporaryTarget = ctx.from ? this.temporaryScreens.get(ctx.from.id) : undefined;
    const isTemporaryTarget =
      target !== undefined &&
      temporaryTarget?.chatId === target.chatId &&
      temporaryTarget.messageId === target.messageId;
    if (target) {
      try {
  ...
  async retireTrackedScreens(ctx: Context): Promise<void> {
    ...
    const targets = [
      ...
      .filter(
        (target, index, all) =>
          all.findIndex((other) => other.chatId === target.chatId && other.messageId === target.messageId) === index
      );
  ...
  private async handlePublicChatCallback(
    ...
    if (action === "remove") {
      ...
      await this.renderScreen(
        ctx,
        `Remove ${this.publicChatLabel(managed)} from managed public chats? Historical moderation records will be preserved.`,
        new InlineKeyboard()
    ...
    if (action === "check" || action === "enable") {
      try {
        ...
        await ctx.answerCallbackQuery({
          text:
            action === "enable" && result.valid
              ? "Moderation enabled."
              : result.valid
                ? "Permissions are healthy."
        ...
        await this.showPublicChatSettings(
          ctx,
          chatId,
          "The public chat could not be inspected. Check the bot membership and permissions, then try again."
        );
      }
      return true;
    }
  ...
  private async handleQuickRepliesCallback(
    ctx: Context,
    action: string | undefined,
    ...
    if (action === "view" && value) {
      this.pendingQuickReplyInputs.delete(ctx.from.id);
      await ctx.answerCallbackQuery();
      await this.showQuickReplyDetail(ctx, value);
      return true;
    }
    if ((action === "edit-name" || action === "edit-text") && value) {
      if (!dependencies.quickReplies.findTemplate(value)) {
        await ctx.answerCallbackQuery({ text: "This Quick Reply is no longer available.", show_alert: true });
        return true;
      }
      await ctx.answerCallbackQuery();
      await this.beginQuickReplyInput(ctx, {
        kind: action === "edit-name" ? "EDIT_TITLE" : "EDIT_TEXT",
        templateId: value,
      });
      return true;
    }
    if (action === "add") {
      ...
      if (categories.length === 1)
        await this.beginQuickReplyInput(ctx, { kind: "ADD_TITLE", categoryId: categories[0]!.id });
      else await this.showQuickReplyCategoryPicker(ctx);
      return true;
    }
    if (action === "add-category" && value) {
      if (!dependencies.quickReplies.listCategories().some((category) => category.id === value)) {
  ...
  private async handleSupportSettingsCallback(ctx: Context, action: string | undefined): Promise<boolean> {
    ...
    await ctx.answerCallbackQuery();
    if (action === "edit") {
      await this.beginSupportResponseTimeInput(ctx);
      return true;
    }
    if (action === "edit-acknowledgement") {
      await this.beginSupportAcknowledgementInput(ctx);
      return true;
    }
    if (action === "reset-response-time") {
      const rendered = validateRenderedSupportAcknowledgement(
        this.supportTicketReceivedTemplate(),
        DEFAULT_SUPPORT_EXPECTED_RESPONSE_TIME
      );
      if (rendered.error) {
        await this.showSupportSettings(
          ctx,
          "Cannot reset response time because the current acknowledgement would become too long. Shorten or reset the acknowledgement first."
        );
        return true;
      }
  ...
  private async handleTeamCallback(
    ctx: Context,
    action: string | undefined,
    value: string | undefined,
    ...
    if (!isPrivateChat(ctx) || !ctx.from) {
      await ctx.answerCallbackQuery({ text: "Private staff dashboard only.", show_alert: true });
      return true;
    }
    if (!(await this.operatorDependenciesOrThrow().canUsePermission(ctx, "MANAGE_TEAM"))) {
      await ctx.answerCallbackQuery({ text: "Your application role cannot manage the team.", show_alert: true });
      return true;
    }
    try {
      if (action === "list") {
      ...
      if (action === "set") {
        ...
        const member = this.installation.getMember(Number(value));
        if (member) await this.showTeamMember(ctx, member);
        return true;
      }
      if (action === "revoke") {
      ...
      await this.retireScreens(ctx);
      await ctx.reply(
        `One-use ${role} invitation (30 minutes):\nhttps://t.me/${this.operatorDependenciesOrThrow().botUsername()}?start=team_${token}`
      );
      await this.sendFreshScreen(ctx, "Team", this.teamKeyboard(ctx.from.id));
    } catch (error) {
      await ctx.answerCallbackQuery({
        text: error instanceof Error ? error.message : "Invitation denied.",
        show_alert: true,
      });
    }
    return true;
  ...
  private async sendPublicChatPicker(ctx: Context): Promise<void> {
    ...
    await this.rememberPublicChatPickerPrompt(
      ctx.from.id,
  ...
  private async beginPublicChatConfiguration(
    ctx: Context,
    chat: ManagedPublicChatRecord,
    ...
    await this.sendFreshScreen(
      ctx,
      this.publicChatConfigurationPrompt(chat, field),
  ...
  private async consumePublicChatConfiguration(ctx: Context, text: string): Promise<boolean> {
    ...
    if (pending.field === "warning") {
      if (!trimmed || trimmed.length > 500) {
        await this.renderScreen(
          ctx,
          this.publicChatConfigurationPrompt(chat, pending.field, "use 1-500 characters."),
          new InlineKeyboard().text("Back to settings", `public:open:${chat.chat_id}`)
        );
        return true;
      }
      ...
      if (entries.length > 100 || entries.some((entry) => entry.length > 80)) {
        await this.renderScreen(
          ctx,
          this.publicChatConfigurationPrompt(chat, pending.field, "use at most 100 terms, each up to 80 characters."),
          new InlineKeyboard().text("Back to settings", `public:open:${chat.chat_id}`)
        );
        return true;
      }
      ...
      if (!Number.isSafeInteger(parsed) || parsed < 1 || parsed > maximum) {
        await this.renderScreen(
          ctx,
          this.publicChatConfigurationPrompt(chat, pending.field, `enter a whole number from 1 to ${maximum}.`),
          new InlineKeyboard().text("Back to settings", `public:open:${chat.chat_id}`)
        );
        return true;
      }
    ...
    dependencies.db.updateManagedPublicChatConfig(chat.chat_id, {
      warningText,
      allowlist,
  ...
  private dashboardText(userId: number): string {
    ...
    return [
      member?.role === "OWNER" ? "Owner dashboard" : `${member?.role.replace("_", " ") ?? "Staff"} dashboard`,
      "",
      this.installation.getState().setupState === "READY" ? "Support is ready." : "Finish setup to activate support.",
      `Public chats: ${counts.publicChats}`,
    ].join("\n");
  }
  ...
  private dashboardKeyboard(userId: number, role: string): InlineKeyboard {
    ...
    if (role === "OWNER" || role === "ADMIN") {
      ...
      keyboard
        .text("Staff workspace", "dashboard:workspace")
        .row()
        .text("Public chats", "dashboard:public")
        .text("Team", "dashboard:team")
        .row()
        .text("Moderation", "dashboard:moderation")
        .text("Quick replies", "dashboard:quick")
        .row()
        .text("Support settings", "dashboard:support")
        .row()
        .text(
          pendingBatch ? "Continue batch" : "Export tickets",
          pendingBatch ? "batch-ui:continue" : "batch-ui:export"
        )
  ...
  private systemStatusText(): string {
    ...
    return [
      "System status",
      "",
      `Bot: @${dependencies.botUsername() ?? "loading"}`,
      `Version: ${dependencies.packageVersion}`,
      `Setup: ${this.installation.getState().setupState}`,
      `Authorization: ${this.installation.getState().authorizationMode}`,
      `Owner: ${this.installation.getOwner()?.username ? `@${this.installation.getOwner()?.username}` : (this.installation.getOwner()?.userTelegramId ?? "not paired")}`,
      `Staff workspace: ${workspace?.title ?? workspace?.telegram_chat_id ?? "not configured"}`,
      `Support Logs: ${workspace && dependencies.db.getSetting(`support_logs_message_thread_id:${workspace.telegram_chat_id}`) ? "configured" : "not configured"}`,
      `Public chats: ${counts.publicChats}`,
      `Team: OWNER ${roles.get("OWNER") ?? 0}, ADMIN ${roles.get("ADMIN") ?? 0}, SENIOR_AGENT ${roles.get("SENIOR_AGENT") ?? 0}, AGENT ${roles.get("AGENT") ?? 0}`,
      `Moderation enabled: ${counts.moderationEnabled}/${counts.publicChats}`,
      `Unhealthy moderation chats: ${counts.unhealthyModerationChats}`,
      `Pending moderation cleanup: ${counts.pendingCleanup}`,
      `Pending archives: ${counts.pendingArchives}`,
  ...
  private publicChatLabel(chat: ManagedPublicChatRecord | undefined): string {
    return chat?.title ?? (chat?.username ? `@${chat.username}` : chat ? String(chat.chat_id) : "Unknown public chat");
  }
  private publicChatButtonLabel(chat: ManagedPublicChatRecord): string {
    const label = this.publicChatLabel(chat);
    return label.length > 40 ? `${label.slice(0, 39)}...` : label;
  }
  private publicChatConnectionLabel(chat: ManagedPublicChatRecord): string {
    return chat.connection_status === "CONNECTED" ? "yes" : chat.connection_status === "UNREACHABLE" ? "no" : "unknown";
  }

  private publicChatConfigurationPrompt(
    chat: ManagedPublicChatRecord,
    field: PublicChatConfigurationField,
    error?: string
  ): string {
  ...
  private teamMemberLabel(member: TeamMemberRecord): string {
    return member.username ? `@${member.username}` : member.display_name?.trim() || "Unnamed member";
  }
  private teamRoleLabel(role: TeamMemberRecord["role"]): string {
  ...
  private teamKeyboard(actorId: number): InlineKeyboard {
    ...
    for (const member of this.installation.listTeamMembers())
      keyboard
        .text(
          `${this.teamRoleLabel(member.role)}: ${this.teamMemberLabel(member)}`,
          `team:member:${member.user_telegram_id}`
        )
        .row();
    if (this.installation.getMember(actorId)?.role === "OWNER")
      keyboard.text("Transfer ownership", "team:transfer").row();
    return keyboard.text("Back", "dashboard:home");
  }
  private async showTeam(ctx: Context): Promise<void> {
    if (!ctx.from) return;
    const members = this.installation.listTeamMembers();
    await this.renderScreen(
      ctx,
      [
        "Team",
        "",
        ...members.map((member) => `${this.teamRoleLabel(member.role)}: ${this.teamMemberLabel(member)}`),
      ].join("\n"),
  ...
  private async showTeamMember(ctx: Context, member: TeamMemberRecord): Promise<void> {
    if (!ctx.from) return;
    const actor = this.installation.getMember(ctx.from.id);
    const keyboard = new InlineKeyboard();
    const text = [
      "Team member",
      "",
      `Member: ${this.teamMemberLabel(member)}`,
      `Role: ${this.teamRoleLabel(member.role)}`,
  ...
  private quickReplyInputPrompt(
    pending: Exclude<PendingQuickReplyInput, { kind: "ADD_PREVIEW" }>,
    error?: string
  ): string {
    const prompt =
      pending.kind === "EDIT_TITLE"
        ? "Send the new reply name (1-32 characters)."
        : pending.kind === "EDIT_TEXT"
          ? "Send the new reply text (1-3500 characters)."
          : pending.kind === "ADD_TITLE"
            ? "Send the new reply name (1-32 characters)."
  ...
  private supportExpectedResponseTime(): string {
    return (
      this.operatorDependenciesOrThrow().db.getSetting("support_expected_response_time")?.trim() ||
      DEFAULT_SUPPORT_EXPECTED_RESPONSE_TIME
    );
  }
  ...
  private supportTicketReceivedTemplate(): string {
    return (
      this.operatorDependenciesOrThrow().db.getSetting("support_ticket_received_template")?.trim() ||
      DEFAULT_SUPPORT_TICKET_RECEIVED_TEMPLATE
    );
  }
  ...
  private supportSettingsText(notice?: string): string {
    return [
      ...
      this.supportExpectedResponseTime(),
      "",
      "New-ticket acknowledgement preview:",
      truncate(formatTicketReceived(this.supportTicketReceivedTemplate(), this.supportExpectedResponseTime()), 1200),
      "",
  ...
  private supportAcknowledgementPrompt(error?: string): string {
    return [
      ...
      "",
      "Current template:",
      truncate(this.supportTicketReceivedTemplate(), 2600),
      "",
  ...
  private normalizeSupportTicketReceivedTemplate(value: string): { value?: string; error?: string } {
    ...
    if (/[\u0000-\u0009\u000b\u000c\u000e-\u001f\u007f]/.test(normalized))
      return { error: "remove unsafe control characters." };
  ...
  private async deleteConsumedSupportSettingsInput(ctx: Context): Promise<void> {
    ...
    try {
      ...
      logger.warn(
        { userId: ctx.from?.id, messageId: ctx.message.message_id },
        "Could not delete consumed support settings input"
      );
    }
  }
  ...
  private rememberScreen(ctx: Context, target: PrivateUiTarget): void {
    ...
    if (!this.installation.getOnboardingSession(ctx.from.id))
      this.installation.saveOnboardingStage(ctx.from.id, "WELCOME", "COMPLETED");
    ...
    return [callbackTarget, trackedTarget, temporaryTarget, persistedTarget]
      ...
      .filter(
        (target, index, targets) =>
          targets.findIndex((other) => other.chatId === target.chatId && other.messageId === target.messageId) === index
      );
  }
  ...
  private async retireScreenTarget(ctx: Context, target: PrivateUiTarget): Promise<void> {
    try {
      ...
      try {
        await ctx.api.editMessageReplyMarkup(target.chatId, target.messageId, {
