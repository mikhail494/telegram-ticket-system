...
async function main(): Promise<void> {
  if (!process.stdin.isTTY || !process.stdout.isTTY)
    throw new Error("Owner recovery requires an interactive terminal confirmation.");
  const prompt = readline.createInterface({ input: process.stdin, output: process.stdout });
  const answer = await prompt.question(
    "Create a one-use owner recovery link? Existing ownership remains active until Telegram confirmation. Type RECOVER: "
  );
  prompt.close();
