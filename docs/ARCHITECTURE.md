...
- **RBAC and installation state:** `InstallationService` manages setup state, owner pairing, team roles, workspace membership, and authorization mode. Private operator UI screens are tracked so callbacks can be rejected when stale.
...
The application favors a compact single-process deployment: `createBot` is currently a large composition closure and `SupportDatabase` spans several domains. Some short-lived interaction state, such as active picker or input sessions, remains in memory while durable business outcomes are persisted in SQLite. These choices reduce moving parts for the current operating model, but they make navigation and future scaling harder.
