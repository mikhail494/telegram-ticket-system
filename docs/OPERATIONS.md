...
Signal handlers are installed before asynchronous startup recovery begins. Startup handles at most 50 durable archive or moderation recovery items, or 10 seconds of recovery work, whichever comes first; Ticket Batch recovery is deferred to the same continuation after polling begins. The time budget is checked only between durable item boundaries: an already-started Telegram or SQLite operation is allowed to finish. Remaining durable recovery is resumed in bounded, instance-owned background chunks after polling begins; durable pending state remains recoverable across another restart.
