...
Each successful backup removes only the temporary database, checksum, WAL, and SHM sidecars created for that attempt. It does not sweep or delete unrelated older temporary files in an operator-managed backup directory.
